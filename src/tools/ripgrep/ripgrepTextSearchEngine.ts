...
import { ripgrepPath } from './rgPath';
import { CancellationToken, Uri, Range, Progress } from 'vscode';
import {
...
import { AST as ReAST, RegExpVisitor, RegExpParser } from 'vscode-regexpp';
import { coalesce } from './array';
import { groupBy } from './collection';

const rgDiskPath = ripgrepPath();
...
export class RipgrepTextSearchEngine {
  ...
  provideTextSearchResults(
    ...
    return new Promise((resolve, reject) => {
      ...
      rgProc.on('error', (e: { message: any }) => {
        console.error(e);
        this.outputChannel.appendLine('Error: ' + (e && e.message));
        reject(
          serializeSearchError(
            new SearchError(e && e.message, SearchErrorCode.rgProcessError)
          )
        );
      });
      ...
      rgProc.stdout!.on('data', (data: string | Buffer) => {
      ...
      let stderr = '';
      rgProc.stderr!.on('data', (data: { toString: () => any }) => {
      ...
      rgProc.on('close', () => {
        ...
        if (isDone) {
          ...
          if (
            ...
            reject(
              serializeSearchError(
                new SearchError(searchError.message, searchError.code)
              )
            );
          } else {
            resolve({ limitHit });
...
export function getRgArgs(
  ...
  if (otherIncludes && otherIncludes.length) {
    ...
    otherIncludes.forEach((other: string) => {
  ...
  if (doubleStarIncludes && doubleStarIncludes.length) {
    doubleStarIncludes.forEach((globArg: string) => {
      args.push('-g', globArg);
    });
  }
...
function spreadGlobComponents(globComponent: string): string[] {
  ...
  return globComponentWithBraceExpansion.flatMap((globArg) => {
    ...
    return components.map((_: any, i: number) =>
...
export function fixRegexNewline(pattern: string): string {
  ...
  const visitor = new RegExpVisitor({
    ...
    onQuantifierEnter(node: any) {
      context.unshift(node);
    },
    onQuantifierLeave() {
      context.shift();
    },
    onCharacterClassRangeEnter(node: any) {
      context.unshift(node);
    },
    onCharacterClassRangeLeave() {
      context.shift();
    },
    onCharacterClassEnter(node: any) {
      context.unshift(node);
    },
    onCharacterClassLeave() {
      context.shift();
    },
    onAssertionEnter(node: any) {
      if (isLookBehind(node)) {
        context.push(node);
      }
    },
    onAssertionLeave(node: any) {
      if (context[0] === node) {
        context.shift();
      }
    },
      ...
      case '{':
        if (escaped) {
          ...
          if (inBraces) {
            ...
            return {
              strInBraces:
                fixedStart + '{' + strInBraces + '{' + pattern.substring(i + 1),
            };
          } else {
            inBraces = true;
          }
...
export function performBraceExpansionForRipgrep(pattern: string): string[] {
  ...
  return arr.flatMap((elem: string) => {
